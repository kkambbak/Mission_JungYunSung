    ...
    CommandLineRunner initData(
        ...
        return new CommandLineRunner() {
            ...
            public void run(String... args) throws Exception {
                ...
                Member memberUser9ByKakao = memberService.whenSocialLogin("KAKAO", "KAKAO__%s".formatted(kakaoDevUserOAuthId)).getData();
                Member memberUser10ByGoogle = memberService.whenSocialLogin("GOOGLE", "GOOGLE__%s".formatted(googleDevUserOAuthId)).getData();
                Member memberUser11ByNaver = memberService.whenSocialLogin("NAVER", "NAVER__%s".formatted(naverDevUserOAuthId)).getData();
                Member memberUser12ByFacebook = memberService.whenSocialLogin("FACEBOOK", "FACEBOOK__%s".formatted(facebookDevUserOAuthId)).getData();
                ...
                //필터링, 정렬 확인용 데이터 추가
                likeablePersonService.like(memberUser1, "insta_user4", 2).getData();
                likeablePersonService.like(memberUser2, "insta_user4", 1).getData();
                likeablePersonService.like(memberUser5, "insta_user4", 3).getData();
                likeablePersonService.like(memberUser6, "insta_user4", 3).getData();
                likeablePersonService.like(memberUser7, "insta_user4", 3).getData();
                likeablePersonService.like(memberUser8, "insta_user4", 3).getData();

                likeablePersonService.like(memberUser8, "insta_user2", 3).getData();
                likeablePersonService.like(memberUser7, "insta_user2", 3).getData();
                likeablePersonService.like(memberUser6, "insta_user2", 3).getData();
                likeablePersonService.like(memberUser8, "insta_user3", 3).getData();
                likeablePersonService.like(memberUser7, "insta_user3", 3).getData();
                likeablePersonService.like(memberUser7, "insta_user1", 3).getData();
            }
        };
    }
}
